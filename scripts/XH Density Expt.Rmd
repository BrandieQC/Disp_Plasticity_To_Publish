...
### Load the data
```{r}
xhdensity <- read.csv("../input/XHDensityData.csv")
head(xhdensity)
```
...
xhheight_cor <- read.csv("../input/XH_HeightCorrection.csv")
...
xhMatFr_summary_fig2 <- xhreptraits_summary_bind %>% 
  ...
  scale_colour_manual(values=ch1Palette_2) +
  facet_grid(. ~Season)
ggsave("../output/XHDensMatFr_NoGenos.png", width=16, height=6, units="in")
```
...
xhBB_summary_fig2 <- xhreptraits_summary_bind %>% 
  ...
  scale_colour_manual(values=ch1Palette_2) +
  facet_grid(. ~Season)
ggsave("../output/XH_BB_NoGenos.png", width=16, height=6, units="in")
```
